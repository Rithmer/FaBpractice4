...
router.get("/", authMiddleware, rolesMiddleware(VIEWER_ROLES), async (req, res) => {
  try {
    ...
    await redisClient.setEx("products:all", PRODUCTS_CACHE_TTL, JSON.stringify(result));
...
router.get("/:id", authMiddleware, rolesMiddleware(VIEWER_ROLES), async (req, res) => {
  try {
...
router.post("/", authMiddleware, rolesMiddleware(SELLER_ROLES), async (req, res) => {
  try {
    ...
    triggerReminders(publicProduct);
...
router.put("/:id", authMiddleware, rolesMiddleware(SELLER_ROLES), async (req, res) => {
  try {
    const product = await Product.findById(req.params.id);
    if (!product) return res.status(404).json({ error: "Товар не найден" });
...
router.delete("/:id", authMiddleware, rolesMiddleware([ROLES.ADMIN]), async (req, res) => {
  try {
